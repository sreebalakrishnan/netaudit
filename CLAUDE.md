...
- Menu bar status glyph (rumps) — a custom Wi-Fi mark tinted by verdict (green/amber/red/gray), bundled from `assets/menubar/` (generated by `generate_icon.py`). Set via `self.icon = path` (the App **property** — NOT `set_icon`, which is a MenuItem method the App lacks and raises AttributeError → falls back to showing the app name); non-template so colour shows, 20pt with the glyph's own padding. Auto-refreshes every 2 min. Emoji 🟢/🟡/🔴 fallback if glyphs missing
...
- `netaudit` CLI: run from a shell for a one-shot terminal verdict (`netaudit`, `--json`, `--no-speed`); `netaudit gui` opens the app. Same py2app executable — `main()` dispatches GUI vs CLI on subcommand / tty. **A py2app bundle can't be launched through a symlink** (bootstrap can't find Info.plist relative to the symlink → `PyRuntimeLocations` error), so the `netaudit` command is a shim script (`cli/netaudit`, bundled to `Contents/Resources/netaudit`, `chmod +x` in build.sh) that execs the real mach-o by its true path. The cask `binary` stanza + `install.sh` symlink point at that shim, never the executable directly
...
| `db.py` | SQLite at `~/.netaudit/network_audit.db` (scans + devices + network_visits, incl. per-network `trusted` flag + `category`/`label`) |
