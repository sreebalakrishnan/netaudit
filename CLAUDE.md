...
- Menu bar status glyph (rumps) — a custom Wi-Fi mark tinted by verdict (green/amber/red/gray), bundled from `assets/menubar/` (generated by `generate_icon.py`, `set_icon` at 18pt non-template so colour shows). Auto-refreshes every 2 min. Emoji 🟢/🟡/🔴 fallback if glyphs missing
